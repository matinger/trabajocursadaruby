...
before do
  ...
  if Resource.all.size < 1
    ...
    Booking.create(start: DateTime.iso8601('2014-02-01T10:00:00Z'), end: DateTime.iso8601('2014-02-01T11:00:00Z'), user_id: user1.id, resource_id: res1.id, status: "approved")
    Booking.create(start: DateTime.iso8601('2014-01-31T10:00:00Z'), end: DateTime.iso8601('2014-01-31T11:00:00Z'), user_id: user1.id, resource_id: res1.id, status: "approved")
    Booking.create(start: DateTime.iso8601('2014-01-31T23:00:00Z'), end: DateTime.iso8601('2014-02-01T09:00:00Z'), user_id: user1.id, resource_id: res1.id, status: "pending")
    Booking.create(start: DateTime.iso8601('2014-02-01T14:00:00Z'), end: DateTime.iso8601('2014-02-01T17:00:00Z'), user_id: user1.id, resource_id: res1.id, status: "approved")
    Booking.create(start: DateTime.iso8601('2014-02-03T14:00:00Z'), end: DateTime.iso8601('2014-02-03T17:00:00Z'), user_id: user1.id, resource_id: res1.id, status: "pending")
    Booking.create(start: DateTime.iso8601('2014-02-02T12:00:00Z'), end: DateTime.iso8601('2014-02-02T13:00:00Z'), user_id: user2.id, resource_id: res2.id, status: "pending")
...
get '/resources/:number/bookings' do
  datetimeconv = DateTime.iso8601(params["date"]+'T00:00:00Z')
...
get '/resources/:number/availability' do
  ...
  datetimeEND = datetimeSTART + params["limit"].to_i
  ...
  if(space_free_init < datetimeEND)
    availability_dates << {from: space_free_init, to: datetimeEND, links:[link_resource(params[:number], false)]}
  end
  JSON.pretty_generate(availability_dates)
end
